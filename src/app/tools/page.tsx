...
import { Bell, Bookmark, Loader2, Trash2, Zap } from 'lucide-react';
...
function ToolsPageContent() {
  ...
  useEffect(() => {
    const fetchData = async () => {
      const [categoryResult, templateResult, filterResult, notificationResult] = await Promise.all([
      ...
      const error = categoryResult.error || templateResult.error || filterResult.error || notificationResult.error;
  ...
  const remove = async (table: 'transaction_templates' | 'saved_filters', id: string, label: string) => {
