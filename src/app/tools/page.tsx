...
function ToolsPageContent() {
  ...
  return <div className="flex flex-col gap-6 px-4 py-5">
    ...
    <section className="flex flex-col gap-3 rounded-3xl border-2 border-slate-800 bg-amber-50 p-4"><h2 className="flex items-center gap-2 font-black"><Tag className="h-5 w-5" />タグ <span className="ml-auto text-xs text-slate-500">{tags.length}/100</span></h2><div className="flex gap-2"><input value={tagName} maxLength={30} onChange={(e) => setTagName(e.target.value)} placeholder="旅行、医療費など" className="min-h-12 min-w-0 flex-1 rounded-xl border-2 border-slate-800 px-3 text-base" /><button onClick={addTag} disabled={mutating || tags.length >= 100} className="min-h-12 rounded-xl border-2 border-slate-800 bg-amber-300 px-4 disabled:opacity-50"><Plus /></button></div>{tags.map((tag) => <div key={tag.id} className="flex items-center justify-between rounded-xl bg-white px-3 py-2 text-sm font-black"><span># {tag.name}</span><button onClick={() => remove('tags', tag.id, `タグ「${tag.name}」`)} disabled={mutating} className="min-h-11 min-w-11 disabled:opacity-50"><Trash2 className="mx-auto h-4 w-4 text-rose-500" /></button></div>)}</section>
    ...
    <section className="flex flex-col gap-3 rounded-3xl border-2 border-slate-800 bg-indigo-50 p-4"><h2 className="flex items-center gap-2 font-black"><Bookmark className="h-5 w-5" />保存した検索条件 <span className="ml-auto text-xs text-slate-500">{filters.length}/100</span></h2>{filters.length === 0 ? <p className="text-xs font-bold text-slate-500">家計簿画面から検索条件を保存できます。</p> : filters.map((item) => <div key={item.id} className="flex items-center justify-between rounded-xl bg-white px-3 py-2 text-sm font-black"><span>{item.name}</span><button onClick={() => remove('saved_filters', item.id, `保存条件「${item.name}」`)} disabled={mutating} className="min-h-11 min-w-11 disabled:opacity-50"><Trash2 className="mx-auto h-4 w-4 text-rose-500" /></button></div>)}</section>
