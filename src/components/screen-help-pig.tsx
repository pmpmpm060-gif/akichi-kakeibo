...
const guides: Record<string, ScreenGuide> = {
  '/': {
    ...
    steps: [
      ...
      '記録カレンダーの日付を押すと、その日の記録を確認できます。',
      '本人プロフィールでは、記録カレンダーの明細を押すと編集・削除できます。',
      'レポート・定期取引・貯金目標は、下部ナビの「その他」から開けます。',
    ],
  ...
  '/dashboard': {
    ...
    steps: [
      ...
      '対象月の収入・支出・収支残高を確認できます。',
      '記録の確認・編集・削除は、ホーム画面の記録カレンダーから行います。',
    ],
    tip: '固定費は「その他」の定期取引から自動登録にできます。',
  },
  '/budgets': {
    title: '予算画面の使い方',
  ...
  '/tools': {
    ...
    tip: 'テンプレートを先に作っておくと、よくある支出をすぐ記録できるぶー！',
  },
  '/login': {
    title: 'ログイン画面の使い方',
